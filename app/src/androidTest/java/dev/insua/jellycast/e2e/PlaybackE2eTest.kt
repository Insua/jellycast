...
class PlaybackE2eTest {
     ...
     * 设计,本意是自动复现现场缺陷,但在**未修复的代码**上实测了五轮**始终是绿的** ——
     * 也就是说,它目前无法把那条旧心跳真正重放到服务端上,因而**证明不了缺陷已修复**。
     *
     * 排查掉的候选(都不是原因,别再重复查):
     * - **serverId 不匹配**:专门写过诊断用例验证,`ProgressReporter` 用的分区和预置行一致,
     *   而且那行记录**确实会被 `flushPending()` 消费掉**(队列 1 条 → 0 条)。
     * - **`playSessionId` 为 null 被服务端忽略**:在真实服务器上做过对照,null 和非空
     *   同样会把位置写回去。
     * - **陈旧位置低于 `MinResumePct`(默认 5%)被服务端丢弃**:已改成按时长取 40%(见
     *   [staleReportPositionMs]),仍然不变红。
     * - **预置时机太早被开头那次 flush 吃掉**:已改成等位置真的前进之后才预置,仍然不变红。
     *
     * 仍未排除的:那次重放的 `reportProgress` 本身失败(于是被重新入队,什么也没写),
     * 或完播 stop 与重放到达服务端的相对顺序和现场不同。要接着查,先加一条能观测
     * 「重放到底发出去了没有、服务端回了什么」的诊断,别再靠改时序猜。
     *
    ...
    fun `播完一集之后服务端不再残留播放位置`() {
        ...
        try {
            ...
            try {
                ...
                // 预置那条「播放途中失败的心跳」—— **必须等到这一集真的响起来之后**。
                //
                // ⚠️ 实测踩过两次的坑:`onSourceReady` 一进来就先 `flushPending()` 一次,而那次
                // 就绪是在 `engine.play()` 返回**之后**由状态流异步触发的。所以「起播前」和
                // 「startPlayback 刚返回」这两个时机都赶不上 —— 记录会被开头那次 flush 直接吃掉,
                // 随后的完播 stop 又把位置清零,于是永远观察不到残留,这条用例在**未修复**的代码上
                // 照样绿,变成一句空话。等到位置真的开始前进,开头那次 flush 必然已经过去了。
                // 真实场景里这条记录本来就是播放途中入队的,这个时机和现场一致。
                assertTrue(
                    "播放从未开始,无法在正确的时机预置补报记录。${diagnostics()}",
                    awaitCondition(STARTUP_TIMEOUT_MS) { position() > 0L },
                )
                runBlocking {
